...
export async function updateSession(request: NextRequest) {
  ...
  // Henderson (2026-09-14): reversed the 2026-09-10 decision above --
  // the root URL should be a real, inviting public landing page for
  // signed-out visitors (hero + Join Now CTA + mission/event content),
  // not a bare redirect straight to the login form. Signed-in visitors
  // still skip straight to /feed, unchanged -- only the signed-out
  // behavior changed, so "/" is no longer in isProtectedRoute below.
